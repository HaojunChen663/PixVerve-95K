...
class BasePipeline(torch.nn.Module):
    ...
    def freeze_except(self, model_names):
        # 1. Global init: set to eval mode and disable all gradients
        self.eval()
        self.requires_grad_(False)
        ...
        for name in model_names:
            ...
            # Here `module` is expected to be an instance of ZImageDiT
            print(f"Configuring partial fine-tuning for model: {name}")

            # ------------------------------------------------------
            # A. Unfreeze all_x_embedder (input projection layer)
            # ------------------------------------------------------
            if hasattr(module, 'all_x_embedder'):
                print(f"  - Unfreezing {name}.all_x_embedder")
                module.all_x_embedder.train()
                module.all_x_embedder.requires_grad_(True)
            
            # ------------------------------------------------------
            # B. Unfreeze local_decoder (MicroDiffusionModel)
            # ------------------------------------------------------
            if hasattr(module, 'local_decoder'):
                print(f"  - Unfreezing {name}.local_decoder")
                module.local_decoder.train()
                module.local_decoder.requires_grad_(True)

            # ------------------------------------------------------
            # C. Unfreeze DiT layers (first 4 and last 4)
            # ------------------------------------------------------
            if hasattr(module, 'layers'):
                num_layers = len(module.layers)
                # Use a set for the indices to avoid duplicates when there
                # are very few layers (e.g. only 6 in total).
                # First-4 indices: 0, 1, 2, 3
                # Last-4 indices: N-4, N-3, N-2, N-1
                indices_to_unfreeze = set(range(4)) | set(range(max(0, num_layers - 4), num_layers))
                
                # Sort for nicer printing
                sorted_indices = sorted(list(indices_to_unfreeze))
                print(f"  - Unfreezing {name}.layers at indices: {sorted_indices}")

                for i in sorted_indices:
                    if i < num_layers:  # safety check
                        layer = module.layers[i]
                        layer.train()
                        layer.requires_grad_(True)
            
            # Note: train() is no longer called on the whole module; it is
            # only called on the unfrozen sub-modules above. This keeps the
            # untrained parts' BatchNorm/Dropout (if any) in eval mode.

        # ------------------------------------------------------
        # Count and report the number of trainable parameters
        # ------------------------------------------------------
        trainable_params = 0
        all_params = 0
        
        # Iterate over all parameters of the pipeline (self)
