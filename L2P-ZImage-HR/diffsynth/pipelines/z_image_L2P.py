...
class ZImagePipeline(BasePipeline):
    ...
    def from_pretrained(
        ...
        # The pixel-space shape must be divisible by the DiT patch size
        # (64 / 128 / 160 for 4K / 8K / 10K). Derive it from the loaded model
        # so that a single pipeline works across all three resolutions.
        if pipe.dit is not None:
            patch_size = pipe.dit.all_patch_size[0]
            pipe.height_division_factor = patch_size
            pipe.width_division_factor = patch_size
    ...
    def pixel_output_to_image(self, pixel_tensor, min_value=-1, max_value=1):
        """
        Convert a pixel-space tensor (B, C, H, W) to a PIL Image.
        Supports an input range of [min_value, max_value].
        """
...
def model_fn_z_image(
    ...
    **kwargs,
    ...
    model_output = dit(
        ...
        timestep,
        prompt_embeds,
        patch_size=dit.all_patch_size[0],
